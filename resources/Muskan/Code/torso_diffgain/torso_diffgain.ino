...
float PIDl, PIDr, pwmLeft, pwmRight, error, previous_error;
float pid_pl=0;
float pid_il=0;
float pid_dl=0;
...
double kpl=25;//3.55
double kil=0.5;//0.005
double kdl=15;//2.05

double kpr=15;//3.55
double kir=0.8;//0.005
double kdr=10;//2.05
...
pid_il = kil*error*elapsedTime;
pid_ir = kir*error*elapsedTime;
...
pid_dl = kdl*((error - previous_error)/elapsedTime);
pid_dr = kdr*((error - previous_error)/elapsedTime);
...
pwmLeft = throttle + PIDl;
pwmRight = throttle - PIDr;
...
void read_mpu_6050_data(){ 
  ...
  AngleRoll = atan(AccY/sqrt(AccX*AccX+AccZ*AccZ))*180/3.142+0.75;
